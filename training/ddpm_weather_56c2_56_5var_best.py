...
# %%
# train_dataset = tf.data.Dataset.from_tensor_slices(((train_data_tf_norm_pred, train_data_tf_norm_past1, train_data_tf_norm_past2), 
#                                                     train_data_tf_norm_pred
#                                                    )
#                                                   )

def train_gen():
    # This yields one sample at a time instead of everything at once
    for i in range(len(train_data_tf_norm_pred)):
        yield (
            (train_data_tf_norm_pred[i], train_data_tf_norm_past1[i], train_data_tf_norm_past2[i]), 
            train_data_tf_norm_pred[i]
        )
...
train_dataset = tf.data.Dataset.from_generator(
    ...
    output_signature=(
        (
            tf.TensorSpec(shape=train_data_tf_norm_pred.shape[1:], dtype=tf.float32),
            tf.TensorSpec(shape=train_data_tf_norm_past1.shape[1:], dtype=tf.float32),
            tf.TensorSpec(shape=train_data_tf_norm_past2.shape[1:], dtype=tf.float32)
        ),
        tf.TensorSpec(shape=train_data_tf_norm_pred.shape[1:], dtype=tf.float32)
    )
...
# val_dataset = tf.data.Dataset.from_tensor_slices(((val_data_tf_norm_pred, val_data_tf_norm_past1, val_data_tf_norm_past2), 
#                                                   val_data_tf_norm_pred
#                                                  )
                                                # )
def val_gen():
    # This yields one sample at a time instead of everything at once
    for i in range(len(val_data_tf_norm_pred)):
        yield (
            (val_data_tf_norm_pred[i], val_data_tf_norm_past1[i], val_data_tf_norm_past2[i]), 
            val_data_tf_norm_pred[i]
        )
...
val_dataset = tf.data.Dataset.from_generator(
    ...
    output_signature=(
        (
            tf.TensorSpec(shape=val_data_tf_norm_pred.shape[1:], dtype=tf.float32),
            tf.TensorSpec(shape=val_data_tf_norm_past1.shape[1:], dtype=tf.float32),
            tf.TensorSpec(shape=val_data_tf_norm_past2.shape[1:], dtype=tf.float32)
        ),
        tf.TensorSpec(shape=val_data_tf_norm_pred.shape[1:], dtype=tf.float32)
    )
)#.batch(32).prefetch(tf.data.AUTOTUNE)
